...
with st.sidebar.expander("Input Data", expanded=True):
    ...
    market_opportunity = st.text_area("Market Opportunity (1-2 sentences)", value="")
    team_highlights = st.text_area("Team Highlights", value="")
    ask_amount = st.number_input("Ask Amount ($M)", value=0.0, min_value=0.1)
    sector = st.text_input("Sector (for AI)", value="your sector")  # For AI prompts
...
with st.sidebar.expander("Financials", expanded=True):
    ...
    if financials.get('dscr') is not None:
        ...
        dscr = st.number_input("DSCR (x) [Optional for Credit]", value=0.0, min_value=0.0)
    if financials.get('irr') is not None:
        irr = st.number_input("IRR (%)", value=financials['irr'], min_value=0.0)
    else:
        irr = st.number_input("IRR (%)", value=0.0, min_value=0.0)

# Revenue Projections
with st.sidebar.expander("Revenue Projections ($M)", expanded=True):
    rev_y1 = st.number_input("Revenue Y1", value=financials.get('revenue_y1', 0.0), min_value=0.0)
    rev_y2 = st.number_input("Revenue Y2", value=financials.get('revenue_y2', 0.0), min_value=0.0)
    rev_y3 = st.number_input("Revenue Y3", value=financials.get('revenue_y3', 0.0), min_value=0.0)
...
with st.sidebar.expander("🤖 AI Text Assist", expanded=False):
    ...
    user_prompt_text = st.text_input("Your Brief/Prompt (e.g., 'your model')", value="Explain business model")
...
with st.sidebar.expander("🖼️ AI Cover Image Gen", expanded=False):
    ...
    user_prompt_image = st.text_input("Cover Prompt (e.g., 'your visuals with charts')", value="sleek visuals over skyline, professional palette")
...
# Footer
st.markdown("---")
st.markdown("*Blank Tester Version: All fields empty—input your data! Next: Real API for images/text? Let's launch.*")